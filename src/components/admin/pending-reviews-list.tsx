...
export async function PendingReviewsList() {
    ...
    return (
      <Card className="p-6">
        <p className="text-gray-500 text-center">
          No pending reviews. Reviews will appear here when customers upload
          data.
        </p>
      </Card>
    );
  ...
  const reviewsWithWaitTime = reviews.map((review) => {
    ...
    return {
      ...
      isUrgent,
      customerName:
        (review.customer_profile as any)?.company_name || "Unknown Customer",
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-2 flex-1">
              ...
              <div className="flex gap-4 text-sm">
                ...
                >
                  ${((review.savings_identified || 0) / 1000).toFixed(0)}K
                  savings
                </Badge>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" asChild>
                <Link href={`/admin/review/${review.id}`}>Quick View</Link>
              </Button>
              <Button size="sm" asChild>
                <Link href={`/admin/review/${review.id}`}>Review Analysis</Link>
              </Button>
            </div>
          </div>
        </Card>
